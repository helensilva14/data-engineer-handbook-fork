...
def do_team_vertex_transformation(spark, dataframe):
    dataframe.createOrReplaceTempView("device_activity")
...
def main():
    spark = SparkSession.builder \
        .master("local") \
        .appName("device_activity_datelist") \
        .getOrCreate()
    output_df = do_team_vertex_transformation(spark, spark.table("device_activity"))
    output_df.write.mode("overwrite").insertInto("device_activity_datelist")